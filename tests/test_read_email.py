...
def test_cli_main_prints_code(monkeypatch, capsys):
    import read_email as module

    def fake_get_latest(folder, pattern):
        assert folder == "variable 1"
        assert pattern == r"\d+"
        return "445566"
    ...
    assert captured.out.strip() == "445566"
    assert captured.err == ""


def test_cli_main_exits_with_error(monkeypatch, capsys):
    import read_email as module
    ...
    captured = capsys.readouterr()
    assert "no code found" in captured.err
